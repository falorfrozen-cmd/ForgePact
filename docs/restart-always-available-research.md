...
the game before the press, so whether it *is* the gate stays unmeasured. The
round-1 rows are in § Results and § Decision carries round 1's lines. The
owner funded a second round the same day; its static search (§ Static search,
Round 2), its instrument (§ Instrument) and its procedure (§ Live procedure,
Round 2) are written, and round 2's live session is pending.
...
- **shipRoute** - which shape the ship code takes: `draw-write` (the value
  ...
  call), or `none` (stop and ask the owner).
...
the step phase; instances step in creation order and the player exists long
before the pause menu, so if the gate is read in a step event the order per
frame is the player's step (recompute), the button's step (read), then the
draw (our write). The hold measures that instead of assuming it: the value
...
nothing else. Round 2's rows (C3, C4, S1 to S7) are added when its session
has run.
