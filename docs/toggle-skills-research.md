...
- **Read:** `instance_number(asset_get_index(GetObjectName(GameObject::White_Mage_Soul_Spurn_AOE_obj))) > 0`.
  ...
  calls landed at room attach, frame 5400, before any Soul Spurn cast existed
  (the first ON evidence is frame 20550), so `0` was the only correct answer
  either could give (`firstHudSpurnInstances=0 firstHudAbilityInstances=0`,
  printed unchanged in three later `tgprobe show` replies — session2.log
  lines 127, 224, 1255, alongside a rising `hudSinceRoomChange` that is a
  separate per-frame counter, not a re-sample). See the Q3-D row for the full
  comparison.
