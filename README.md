...
| **Timed skill countdown** | For a cast of Healing Zone (White Mage), Blade Barrier (Samurai), Soul Spurn (White Mage) or Maelstrom of Frost (Prophet) - the four timed skills measured in-game so far: draws how much of the cast is left over its skill-bar slot, in one of four looks (arc / bar / number / fade), disappearing at zero. In a fight, each hit adds a little time to Blade Barrier (about 0.2 s) and the countdown rises slightly to match; it ends when the blades do. A toggled-on Soul Spurn or Maelstrom of Frost never gets a countdown. Off by default; a cast already running when you turn it on shows as full until the next cast |
