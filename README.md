...
| **Timed skill countdown** | For a cast of Healing Zone (White Mage), Blade Barrier (Samurai), Soul Spurn (White Mage) or Maelstrom of Frost (Prophet) - the four timed skills measured and tested in-game - or any other skill with both a duration and a cooldown, covered by rule and untested: draws how much of the cast is left over its skill-bar slot, in one of four looks (arc / bar / number / fade), disappearing at zero. Companion skills (turrets, totems, hydra) and skills whose duration is only a buff on you are not covered. In a fight, hits can add a little time to Blade Barrier (roughly 0.2 s each in our test) and the countdown rises slightly to match; it ends when the blades do. A toggled-on Soul Spurn or Maelstrom of Frost never gets a countdown. Off by default; a cast already running when you turn it on shows as full until the next cast |
