...
| **Timed skill countdown** | For a plain (non-toggled) cast of Soul Spurn or Maelstrom of Frost, and Crematus where the game exposes that cast's own timer: draws how much of the cast is left over its skill-bar slot, in one of four looks (arc / bar / number / fade), disappearing at zero. Lunar Orbit and Submerged Knives never show one - their plain cast creates no timed instance for the mod to read, only their toggled-on state does - and a toggled-on skill never gets a countdown either way. Off by default; a cast already running when you turn it on shows as full until the next cast |
