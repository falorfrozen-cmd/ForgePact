...
<div class="card tab-card" data-tab="mods" id="gameplayCard">
  ...
  <div class="row" style="border:none">
    ...
    <div class="row" style="border:none">
        <span class="lbl" style="width:auto;flex:1">Timed skill countdown<br><span style="font-size:11px;color:#8f816e;font-weight:normal">Healing Zone (White Mage), Blade Barrier (Samurai), Soul Spurn (White Mage) and Maelstrom of Frost (Prophet) are measured and tested in-game; any other skill with both a duration and a cooldown is also covered, by rule, and is untested - companion skills (turrets, totems, hydra) and skills whose duration is only a buff on you are not covered. Draws how much of a cast is left over that skill's skill-bar slot, in the look you pick below. In a fight, hits can add a little time to Blade Barrier (roughly 0.2 s each in our test) and the countdown rises slightly to match; it ends when the blades do. A toggled-on Soul Spurn or Maelstrom of Frost never gets a countdown. Off by default; if a cast is already running when you turn this on, it shows as full until it is cast again.</span></span>
